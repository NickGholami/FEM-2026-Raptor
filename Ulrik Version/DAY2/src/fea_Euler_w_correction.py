...
class Fea:
    def __init__(self, input_file):       
        ...
        # Euler loop w. correction
        for i in range(nincr):
            R_int = np.zeros((neqn, 1))
            P = P + deltaP_vec
            Kmatr = sps.csc_matrix((neqn, neqn))
            Kmatr = buildstiff(X, IX, ne, mprop, Kmatr, strain)
            Kmatr, deltaP_BC = enforce(Kmatr, deltaP_vec.copy() - R, bound)
            deltaD = solve_displacements(Kmatr, deltaP_BC)    
            D = D + deltaD
...
def plot_comparison(X, mprop, u_history, P_history, nincr):
    ...
    plt.plot(u_history, P_history, "red", label= f"Euler w. correction - {nincr} load increments")
